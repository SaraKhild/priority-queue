...
public class PriorityUserServcie {
    ...
    public Mono<Void> addUser(User user) {
        return this.sortedSet.add(user.getCategory().ordinal(), user).doOnSuccess(success -> System.out.println("User added: " + success))
        .then();

    }

    public Flux<User> getUser() {
        return sortedSet.takeFirstElements().limitRate(1).doOnNext(System.out::print);
